...
const energyBar = document.getElementById('energy-bar');
const energyBarHol = document.getElementById('energy-bar-holder');
...
const ratingSys = document.getElementById('rating-system');
...
const failureSound = new Audio("sounds/fail.mp3");
const fireSound = new Audio("sounds/fire.mp3");
const nightSound = new Audio("sounds/night.mp3");
const successSound = new Audio("sounds/success1.mp3");
// Audio control
...
const settingsPage = document.getElementById("accHol");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Check for saved color blindness mode in localStorage
    const savedMode = localStorage.getItem('colorBlindMode') || 'default';
    applyColorBlindMode(savedMode);

    // Event listener for color blindness mode dropdown
    const colorBlindDropdown = document.getElementById('color-blindness');
    if (colorBlindDropdown) {
        colorBlindDropdown.value = savedMode; // Set the dropdown to the saved mode
        colorBlindDropdown.addEventListener('change', (e) => {
            const selectedMode = e.target.value;
            applyColorBlindMode(selectedMode);
        });
    }
});
...
function choose(option) {
    ...
    switch (option) {
        ...
        case 'buildFire':
            ...
            if (energyAfterFire > 35) {
                energyBar.style.background = "var(--primary-color)";
            } else if (energyAfterFire > 15) {
                energyBar.style.background = "var(--accent-color)";
            } else {
                energyBar.style.background = "var(--warning-color)";
            }
...
function setupInstructionPage() {
    ...
    if (languageSelect) {
        languageSelect.disabled = true; // This grays it out
        languageSelect.classList.add('disabled-language'); // Optional: for custom styling
    }
...
function setupArrivalPage() {
    ...
    fireAnimation.src = 'island.gif'; // Replace with the new GIF source
    ...
    }, 1500); // 1.5 seconds matches the CSS transition

    // Show energy display    
    energyDisplay.style.display = 'flex';
    energyBar.style.display = 'flex';
    energyBarHol.style.display = 'flex';
...
function setupExploreIslandPage() {
    ...
    const skullRock = document.getElementById('skull-rock-location')
    skullRock.addEventListener('mouseenter', function() {
        document.body.style.backgroundImage = "url('images/skullrock.jpeg')";
        skullRock.style.opacity = "0.5";
      });
    
      skullRock.addEventListener('mouseleave', function() {
        document.body.style.backgroundImage = "none";
        skullRock.style.opacity = "1";
      });
    
      const ghostImage = document.getElementById('ghost-cave-location');
      ghostImage.addEventListener('mouseenter', function() {
        document.body.style.backgroundImage = "url('images/ghostlightcave.jpeg')";
        ghostImage.style.opacity = "0.5";
      });
    
      ghostImage.addEventListener('mouseleave', function() {
        document.body.style.backgroundImage = "none";
        ghostImage.style.opacity = "1";
      });

      const forgotImage = document.getElementById('forgotten-falls-location')
      forgotImage.addEventListener('mouseenter', function() {
        document.body.style.backgroundImage = "url('images/forgottenfalls.jpeg')";
        forgotImage.style.opacity = "0.5";
      });
    
      forgotImage.addEventListener('mouseleave', function() {
        document.body.style.backgroundImage = "none";
        forgotImage.style.opacity = "1";
      });
...
function setupBuildFirePage() {
    ...
    currentEnergy = reportVariables.energyAfterFire;
    ...
    // Update the fire animation GIF dynamically
    fireAnimation.src = 'fire-12470.gif'; // Replace with the new GIF source
...
function setupShelter(shelterImageSrc, shelterType) {
    ...
    // Update the fire animation GIF dynamically
    fireAnimation.src = 'cap-10524.gif'; // Replace with the new GIF source
...
function setupReportPage() {
    ...
    energyBarHol.style.display = 'none';
    ...
    } else if (reportVariables.buildAFire) {
        reportData = [
            { Activity: "Start of Journey", Energy: 50 },
...
function helpButtonClick() {
    ...
    sessionStorage.setItem("energyDisplay", energyDisplay.style.display);
    
    sessionStorage.setItem("energyBarDisplay", energyBar.style.display);
    sessionStorage.setItem("energyBarHolDisplay", energyBarHol.style.display);
    sessionStorage.setItem("energyBarBackground", energyBar.style.background);
    sessionStorage.setItem("energyBarHeight", energyBar.style.height);
    sessionStorage.setItem("energyBarTop", energyBar.style.top);

    sessionStorage.setItem("helpButton", helpButton.style.display);
    sessionStorage.setItem("restartButton", restartButton.style.display);
    sessionStorage.setItem("voiceButton", voiceButton.style.display);
...
window.addEventListener("load", function () {
    ...
    if (sessionStorage.getItem("returnFromHelp") === "true") {
        ...
        energyDisplay.style.display = sessionStorage.getItem("energyDisplay");
        helpButton.style.display = sessionStorage.getItem("helpButton");
        restartButton.style.display = sessionStorage.getItem("restartButton");
        voiceButton.style.display = sessionStorage.getItem("voiceButton");

        energyBar.style.display = sessionStorage.getItem("energyBarDisplay");
        energyBarHol.style.display = sessionStorage.getItem("energyBarHolDisplay");
        energyBar.style.background = sessionStorage.getItem("energyBarBackground");
        energyBar.style.height = sessionStorage.getItem("energyBarHeight");
        energyBar.style.top = sessionStorage.getItem("energyBarTop");
        ...
        sessionStorage.removeItem("energyDisplay");
        sessionStorage.removeItem("helpButton");
        sessionStorage.removeItem("restartButton");
        sessionStorage.removeItem("voiceButton");
...
function applyColorBlindMode(mode) {
    // Remove any existing color blindness or high contrast classes
    document.body.classList.remove('deuteranomaly', 'protanomaly', 'tritanomaly', 'high-contrast');
